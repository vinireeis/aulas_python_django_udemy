...
from .models import CadastrarContato
...
def dashboard(request):
    if request.method != 'POST':
        form = CadastrarContato()
        return render(request, 'accounts/dashboard.html', {'form': form})

    form = CadastrarContato(request.POST, request.FILES)

    if not form.is_valid():
        messages.error(request, "Há dados incompletos")
        form = CadastrarContato(request.POST)
        return render(request, 'accounts/dashboard.html', {'form': form})
